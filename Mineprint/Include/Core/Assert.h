...
// @TODO(mware): Assertions are currently not thread safe, and recursive assertions are not safely handled (we should abort if an assert causes an assert!).
...
struct AssertionData
{
    ...
    // We keep an internal linked list of assertion data, so we can generate an assertion report.
    AssertionData* Next;
};
...
// TODO(mware): With only break on spaces, we might get lines longer than maxLineLength if there are no spaces.
// We could improve this by backtracking to the last space if we exceed maxLineLength, or holding a temporary buffer 
// of the current word that we can drop down to a new line if needed.
...
AssertState ReportAssertion(AssertionData* data, const char* message = "Assertion Failed", Args&&... args)
{
    ...
    if (!success)
    {
        MP_ERROR("Couldn't even show a message box: {}\nWe're truly fucked; breaking.", SDL_GetError());
    ...
    case 5:
        // ABORT!!!!!
        MP_ERROR("Aborting due to failed assertion.");
        ShutdownLog();
    ...
    default:
        ...
        MP_WARN("Invalid assertion button pressed: {}. How'd you manage that?", buttonPressed);
        return AssertState::Break;
    }
}
...
inline void PrintAssertionReport()
{
    ...
    if (current == nullptr)
    {
        MP_INFO("No assertions triggered.");
        return;
    }
    ...
    {
        MP_INFO("\"{}\" ({} at {}:{}) - Triggered {} times", current->Condition, current->Function,
...
#define MP_ENABLED_ASSERT(cond, silent, ...) \
	do { \
...
#define MP_DISABLED_ASSERT(cond, ...) \
	do { \
		(void) sizeof((cond)); \
	} while (NULL_WHILE_LOOP_CONDITION)
...
#define MP_CHECK(cond, ...) \
	MP_ENABLED_ASSERT(cond, false, ##__VA_ARGS__)
#define MP_CHECK_SILENT(cond, ...) \
	MP_ENABLED_ASSERT(cond, true, ##__VA_ARGS__)

#ifndef MP_DISABLE_ASSERTS
#define MP_ASSERT(cond, ...) \
	MP_ENABLED_ASSERT(cond, false, ##__VA_ARGS__)
#define MP_ASSERT_SILENT(cond, ...) \
	MP_ENABLED_ASSERT(cond, true, ##__VA_ARGS__)
#else
#define MP_ASSERT(cond, ...) \
	MP_DISABLED_ASSERT(cond, ##__VA_ARGS__)
#define MP_ASSERT_SILENT(cond, ...) \
	MP_DISABLED_ASSERT(cond, ##__VA_ARGS__)
#endif
